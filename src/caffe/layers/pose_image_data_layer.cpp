...
bool PoseReadImageToDatum_mode1(const string& filename, const string &labelfile, const int height, const int width, Datum* datum, const float scale,
		const float torso_ratio, const int mx1, const int mx2, const int my1, const int my2, const int randmargin)
{
  cv::Mat cv_img;
  if (height > 0 && width > 0)
  {
  ...
  datum->clear_float_data();
	...
	// scale
	int height2 = (maxy - miny + 1) * scale;
	int width2 = (maxx - minx + 1) * scale;
	int centx = (minx + maxx) / 2;
	int centy = (miny + maxy) / 2;

	// add margin
	int x1 = max((int)(centx - width2  / 2 - mx1 - randmargin * random(0,1)), 0);
	int y1 = max((int)(centy - height2 / 2 - my1 - randmargin * random(0,1)), 0);
	int x2 = min((int)(centx + width2  / 2 + mx2 + randmargin * random(0,1)), cv_img2.cols - 1);
	int y2 = min((int)(centy + height2 / 2 + my2 + randmargin * random(0,1)), cv_img2.rows - 1);
...
bool PoseReadImageToDatum_mode2(const string& filename, const string &labelfile, const int height, const int width, Datum* datum, const float scale,
		const float torso_ratio, const int mx1, const int mx2, const int my1, const int my2, const int randmargin)
{
  cv::Mat cv_img;
  if (height > 0 && width > 0)
  ...
  datum->clear_float_data();
	...
	tb4 = pts[pts.size() - 1], tb3 = pts[pts.size() - 2], tb2 = pts[pts.size() - 3], tb1 = pts[pts.size() - 4];

	minx = min(minx, (int)(tb1 - mx1 - randmargin * random(0,1)));
	miny = min(miny, (int)(tb2 - my1 - randmargin * random(0,1)));
	maxx = max(maxx, (int)(tb3 + mx2 + randmargin * random(0,1)));
	maxy = max(maxy, (int)(tb4 + my2 + randmargin * random(0,1)));

	minx = max(minx, 0);
	miny = max(miny, 0);
	maxx = min(maxx, cv_img2.cols - 1);
	maxy = min(maxy, cv_img2.rows - 1);
	...
	// add margin
	int x1 = max(centx - width2  / 2, 0);
	int y1 = max(centy - height2 / 2, 0);
	int x2 = min(centx + width2  / 2, cv_img2.cols - 1);
	int y2 = min(centy + height2 / 2, cv_img2.rows - 1);
