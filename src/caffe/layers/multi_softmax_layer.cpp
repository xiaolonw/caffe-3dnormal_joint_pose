...
void MultiSoftmaxLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
...
template<typename Dtype>
Dtype MultiSoftmaxLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top)
{
...
/*
	// subtraction
	// top_data(num,:) = top_data(num,:) - scale_data(num,1) * sum_multiplier_(1,:)
	caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num , dim, 1, -1.,
			scale_data, sum_multiplier_.cpu_data(), 1., top_data);
	// Perform exponentiation
	caffe_exp<Dtype>(num * dim, top_data, top_data);
	// sum after exp
	// scale_data(num,1) = top_data(num, :) * sum_multiplier_(:,1)
