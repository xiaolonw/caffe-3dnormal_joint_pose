...
template <typename Dtype>
void Net<Dtype>::BruteCopyTrainedLayersFrom(const NetParameter& param) {
  int num_source_layers = param.layers_size();
  for (int i = 0; i < num_source_layers; ++i) {
    const LayerParameter& source_layer = param.layers(i);
    const string& source_layer_name = source_layer.name();
    ...
    CHECK_EQ(target_blobs.size(), source_layer.blobs_size())
        << "Incompatible number of blobs for layer " << source_layer_name;
    for (int j = 0; j < target_blobs.size(); ++j) {
      //CHECK_EQ(target_blobs[j]->count(), source_layer.blobs(j).count());
      target_blobs[j]->BruteFromProto(source_layer.blobs(j));
    }
  }
}
