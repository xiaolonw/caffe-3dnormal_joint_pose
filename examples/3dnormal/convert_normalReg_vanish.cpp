...
#define RESIZE_WIDTH 55
#define LABEL_LEN 9075
    ...
    const int height, const int width, Datum* datum)
{
	...
	//sprintf(ss2,"/home/dragon123/cnncode/3ddata/testReg/%c%c_edge.jpg",filename[filename.size() - 10],filename[filename.size() - 9]);
...
int main(int argc, char** argv) {
	...
	for (int anno_id = 0; anno_id < annos.size(); ++anno_id)
	{
		...
		labelFile[labelFile.size() - 2] = 'x';
		labelFile[labelFile.size() - 3] = 't';
		if (!MyReadImageToDatum(root_folder + "/" + annos[anno_id].filename_, coarse_folder + "/" + labelFile, edge_folder + "/" + labelFile,
