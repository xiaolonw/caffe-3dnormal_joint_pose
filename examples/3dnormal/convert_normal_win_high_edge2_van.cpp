...
bool MyReadImageToDatum(const string& filename, const string &filename2, const string &filename3, const string &filename4, const string &filename5,    const std::vector<float> & label,
    const int height, const int width, Datum* datum)
{
	...
	//sprintf(ss2,"/home/dragon123/cnncode/3ddata/testReg/%c%c_van.jpg",filename[filename.size() - 14],filename[filename.size() - 13]);
	pFile = fopen(filename4.c_str(), "rb");
	for (int c = 0; c < 3; ++c)
	{
		for (int h = 0; h < cv_img.rows; ++h)
		{
	...
	fclose(pFile);
	//imwrite(ss2,imgIn);
pFile = fopen(filename5.c_str(), "rb");
for (int c = 0; c < 3; ++c)
{
	for (int h = 0; h < cv_img.rows; ++h)
	{
...
int main(int argc, char** argv) {
	...
	for (int anno_id = 0; anno_id < annos.size(); ++anno_id)
	{
		...
		if (!MyReadImageToDatum(root_folder + "/" + annos[anno_id].filename_, coarse_folder + "/" + labelFile, local_folder + "/" + labelFile, van_folder + "/" + labelFile,
				edge_folder + '/' + labelFile, annos[anno_id].pos_, height, width, &datum))
		{
			continue;
		}
