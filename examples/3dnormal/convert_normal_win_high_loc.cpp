...
bool MyReadImageToDatum(const string& filename, const string &filename2, const string &filename3, const string &filename4, const std::vector<float> & label,
    const int height, const int width, Datum* datum)
{
	...
	char ss2[1010];
//	Mat imgIn(Size(55,55),CV_8UC3);
//	sprintf(ss2,"/home/dragon123/cnncode/3ddata/testReg/%c%c_loc.jpg",filename[filename.size() - 14],filename[filename.size() - 13]);
	...
	for (int c = 0; c < 2; ++c)
	{
		for (int h = 0; h < cv_img.rows; ++h)
		{
		  for (int w = 0; w < cv_img.cols; ++w)
		  {
			...
			fread(&tnum, sizeof(float), 1, pFile);
			datum->add_float_data(tnum );
//			imgIn.at<cv::Vec3b>(h, w)[0] = (uchar)(tnum);
//			imgIn.at<cv::Vec3b>(h, w)[1] = (uchar)(tnum);
//			imgIn.at<cv::Vec3b>(h, w)[2] = (uchar)(tnum);
		  }
		}
	}
	fclose(pFile);
