...
bool MyReadImageToDatum(const string& filename, const string& filename2, const std::vector<int> & label,
    const int height, const int width, Datum* datum)
{
	...
	string filenamex = filename2 + "_x.jpg";
	string filenamey = filename2 + "_y.jpg";
	cv::Mat cv_x = cv::imread(filenamex, CV_LOAD_IMAGE_GRAYSCALE);
	cv::Mat cv_y = cv::imread(filenamey, CV_LOAD_IMAGE_GRAYSCALE);
	for (int h = 0; h < cv_img.rows; ++h)
	{
		for (int w = 0; w < cv_img.cols; ++w)
		{
			int t = cv_x.at<uchar>(h, w);
			datum_string->push_back(cv_x.at<uchar>(h, w));
		}
	}
	for (int h = 0; h < cv_img.rows; ++h)
	{
		for (int w = 0; w < cv_img.cols; ++w)
		{
			datum_string->push_back(cv_y.at<uchar>(h, w));
		}
	}
